...
import { IGetStates, IModelDefinitions } from './type'
import { StoreApi, UseBoundStore, create } from 'zustand'

const boundStore: {
  [modelName: string]: UseBoundStore<
    StoreApi<{ state: any; loadings?: string[] }>
  >
} = {}

const getStates: IGetStates<{ [P in string]: any }> = (
  modelName,
  callbackState
) => {
  const currentState =
...
>(
  models: () => (IModelDefinitions<STATE, ACTION> & {
    default: IModelDefinitions<STATE, ACTION>
  })[],
  ...
  for (const modelDefinition of modelDefinitions) {
    const { name, model, subscriptions } =
      typeof modelDefinition.default === 'object' && modelDefinition.default
